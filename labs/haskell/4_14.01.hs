...
sumLast :: Int -> Int -> [Int]
sumLast initial n = [initial, initial] ++ generate [initial, initial]
...
transformSum (Tree root left right) = Tree (root + subtreeSum left + subtreeSum right) (transformSum left) (transformSum right)
...
-- а за всяка следваща цифра 0 означава завиване по левия клон, а 1 — по десния. Да се реализира функция sameAsCode, 
-- която в двоично дърво от числа връща такова число x, което съвпада по стойност с двоичното число, кодиращо пътя 
-- от корена до x, или 0, ако такова число няма. Представянето на дървото е по ваш избор.
